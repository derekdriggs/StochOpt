function [xk1, its, ek, fk, sk, tk, gk] = func_katyusha(para, GradF, iGradF, ObjF, ProxJ)
%
% Uses Katyusha to solve the problem
%
%    min_x   1/n sum_{i=1}^n f_i(x) + mu * J(x)
%
% where f_i has a Lipschitz continuous gradient for all i and J is strongly
% convex. If J is not strongly convex, use Katyusha_ns.
%
% Inputs:
...
%     P          - epoch length (2*m);
%     gamma      - step-size for mirror descent step (0.1);
%     eta        - step-size for gradient descent step (0.1);
%     tau1       - (Nesterov) momentum parameter (0.25);
%     tau2       - (Katyusha) momentum parameter (0.5);
...
while(its<maxits)
    ...
    for p=1:P
        ...
        xk1 = tau1*z + tau2*x_tilde + (1-tau1-tau2)*y;
        ...
        % Update z
        w = z - gamma * mean( Gj_k1 - Gj_k2, 2 ) - gamma * g_full;
        z = ProxJ(w, gamma*mu);
        ...
        if mod(t,objEvery)==0
            l = l+1;
            fk(l) = ObjF(xk1);
            ek(l) = norm(xk1(:)-(tau1*z+tau2*x_tilde+(1-tau1-tau2)*y), 'fro');
            sk(l) = sum(abs(xk1) > 0);
            gk(l) = gamma;
            tk(l) = toc;
